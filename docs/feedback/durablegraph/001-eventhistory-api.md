...
> 状态：已整理，待上游评估。DramaBoard 尚未接入，本文不把评审观察写成真实游戏接入中已发生的故障。
...
本次反馈来自公开 README、API 源码及上一轮实际运行的上游测试/真实包实验；完整验证范围与本地依赖条件见[消费者合同 §6](../../research/event-journal-state-store-draft.md#6-接入边界与最小验证)。本报告形成时上游 HEAD 未变，本轮只核对接口与文档，没有再次运行测试。

| 条目 | 证据性质 | 建议顺序 |
|---|---|---|
| 001-A · 起步示例使用默认保存策略 | 已确认 API 已提供，README 未展示最简调用 | 近期文档改进 |
| 001-B · Commit 的调用处文档与恢复示例 | 已确认合同分散，方法 XML doc 不完整 | 近期文档改进 |
| 001-C · 展示含引用的事件快照建模 | 已确认起步示例未覆盖，误用风险待消费者验证 | 近期示例改进 |
| 001-D · 跨重开的历史记录定位 | 已确认无公开定位入口，具体使用工作量待验证 | 候选 API 需求 |
| 001-E · 最近事件与局部倒序浏览 | 已确认整链物化，长轨迹成本未测量 | 候选 API/性能反馈 |
