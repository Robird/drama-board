...
上游实现与其测试/真实包验收记录集中在 [DB-065](../../../../durable-graph/docs/design-branches/0065-event-history-consumer-contract-slice.md)。本仓库之前实际运行的 `1cace42` 验证范围仍见[消费者合同 §6](../../research/event-journal-state-store-draft.md#6-接入边界与最小验证)；不能将其算作新版复跑证据。
...
命名分支只有在应用保持其不移动时才能固定历史点，不当成不可变书签。D/E 均不阻塞[首个真实接入](../../worksets/durablegraph-first-integration.md)，也没有因文档改进而变成已实现 API。
