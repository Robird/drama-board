...
public sealed class TimerSystemTests {
    ...
    public async Task StepAsync_ThreeTimersCommitOneBatchPerStepInTimeOrder() {
        ...
        var journal = new InMemoryOccurrenceHistory<TimerWorld, TimerFact>(world, new(new(1, 0), ModelTime.Zero, null, null));
        SimulationKernel<TimerWorld, string, TimerFact> kernel =
            TimerModel.CreateKernel(rule, journal);
        ...
        Assert.Equal(StepStatus.Committed, await kernel.StepAsync(AtSecond(20)));
        Assert.Equal(2, journal.CompletedEvents.Count);
        ...
        Assert.Equal([10_000L, 15_000L, 20_000L],
            journal.CompletedEvents.Select(batch => batch.TargetInstant.ModelTime.Ticks));
    ...
    public async Task StepAsync_SameTickTimersUseSchedulerAndReforecastAfterEachCommit() {
        ...
        Assert.Equal([0L, 1L], journal.CompletedEvents.Select(batch => batch.TargetInstant.CausalOrdinal));
        Assert.Equal(kernel.World.FiredTimers, journal.CompletedEvents.Select(batch => batch.Facts[0].TimerName));
    }
