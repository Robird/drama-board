...
**本版:2026-08-17,codex 完成 WP18。状态:第一阶段(WP0–WP11)+ 两轮攻击性评审修复(A1–A10、S1–S14)+ 第二阶段 WP12–WP18 完成;主 slnx 205 / Local.slnx 219 测试绿;下一步 WP19(复用 action.observe 支持目标对象检查)。**

---

## 1. 项目与接手者角色

- 项目:DramaBoard(戏剧棋盘)——AI Player 参与的沙盒戏剧棋类。已建成:确定性 event-sourced Simulation Kernel + Protocol DTO + Host 决策编排 + FirstBoard 场景 + atelia 落盘 adapter + 双真后端 LLM Player + 可运行戏剧记录 demo。当前目标:让现有 observe 能检查具体对象，在不扩动作词汇表的前提下补上真伪/内容验证。
...
2. `docs/研发计划_005_LLM_Player设计研究.md` — **当前阶段执行依据**:三大设计问题(Observation/Action/内部状态)的裁决与依据、后端接入与成本策略、WP13–WP19 路线。
...
5. `git log --oneline -20`;干活前跑 `dotnet test DramaBoard.slnx` 确认基线(205 绿;Local.slnx 219 绿,含 atelia 落盘集成)。
...
- **信念/反应层**(α+WP16–WP18):affordance 只由 actor.KnownFacts 计算;拒绝与每次完成事件投影单值 `action.last-outcome`;placed/carried 可见性(携带物仅持有者可见);`object.held` 明示自持物。talk 向听者写最后一条 `dialogue.heard` 并中断 wait。`give` 是非原子单物品转移,接收者写 `object.received`;`show` 只允许同场持有者展示,目标写 `object.shown` 历史证据并中断 wait,不转所有权。不做契约/公平保证,所以先付、履约与背叛都是有效戏剧结果。原测试项目现只放测试/RecordingPlayerDriver;落盘测试直接引用正式项目。
...
### Player.Llm(src/Player.Llm,WP13–WP18,零 NuGet,引用 Protocol+Host)
...
- **FirstBoard.Demo**:BCL-only console;Alice/Bob 可用 `--alice-backend/model`、`--bob-backend/model` 独立选择后端,相同 endpoint 仍共享实例;其余端点/env key/超时/turn 预算共用配置。每成功 turn 立即写 memory snapshot+turns.md,结束写中文 journal+内心轨迹。WP17/18 已跑真实混合场;输出在 git ignored `artifacts/wp15/`–`wp18/`。

## 4. 当前状态与后续计划(交接核心)

- **已完成**:WP0–WP18 全部;A1–A10 + S1–S14 两轮评审修复全清。关键 commit:δ=cd75fcc、α=992c5a2、β=a3f78a2、γ=392a63d、WP12=b16ca21、WP13=c5307a2、WP14=f3d0721、WP15=930ba78、WP16=d8d06ae、WP17=e5fd4ec;WP18 commit 见当前 HEAD。
...
### 下一步:WP19 目标化观察(复用 action.observe)

WP18 实现与真场资料:
- `action.show` 复用 give 的持有/同场校验;权威 `object.shown` 只向目标投影“事件时点确实持有”的私有事实,不转所有权、不把历史证据伪装成实时状态。展示像 talk 一样中断 wait,使目标可立即反应。
- 脚本场覆盖 affordance、私有投影、中断、所有权不变。混合场:Alice=DeepSeek v4-flash,Bob=GPT-5.6 Luna low,89 events/24 turns/约 363s;Alice 自主 show 第一枚币,Bob 明确认知“确有但未交付”,Alice 随后 give 一币,Bob 履约 give 密信。终局 Alice=letter+coin2,Bob=key+coin1,与 WP17 同 seed 欺诈局形成鲜明对照。
- 一次 Luna 产生裸 `talk` 而非 `action.talk`,领域拒绝后下一决策自行恢复;这是可观察的小瑕疵,暂不抢占表达力主线。现有 DeepSeek key/base URL 位于 Windows 用户级环境变量,新 Codex 进程可能不自动继承;Demo 子进程读取用户级配置即可,无需增加 key。

WP19 方向(未否决按倾向推进):
1. Alice 收信后自然提出“验证真伪/内容”,但无目标 observe 只能检查房间。A=prompt 声明 ID 已充分验证;B=复用 `action.observe(TargetObjectId)`,允许检查自持/现场可见对象并获得对象特有私有 fact;C=新增 inspect/read。**倾向 B**,保持最小动作集合。
2. 真模型验收:Alice 对持有的 `duchess-letter` 做目标观察→获得真伪/内容证据→自主给第二枚币或违约。仍不解析契约、不强制履约。
3. `show` 已成功改变议价轨迹,scene cue/Director 继续后置。005 §6 既定倾向仍有效:独白向观众展示、prompt/记忆用中文。
