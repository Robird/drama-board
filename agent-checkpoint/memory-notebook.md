...
**本版:2026-08-17,codex 完成 WP19。状态:第一阶段(WP0–WP11)+ 两轮攻击性评审修复(A1–A10、S1–S14)+ 第二阶段 WP12–WP19 完成;主 slnx 206 / Local.slnx 220 测试绿;下一步 WP20(不可变 Scene Briefing)。**

---

## 1. 项目与接手者角色

- 项目:DramaBoard(戏剧棋盘)——AI Player 参与的沙盒戏剧棋类。已建成:确定性 event-sourced Simulation Kernel + Protocol DTO + Host 决策编排 + FirstBoard 场景 + atelia 落盘 adapter + 双真后端 LLM Player + 可运行戏剧记录 demo。当前目标:把稳定场景前提从会被 LLM 整体替换的记忆中分离，形成不可变 Scene Briefing。
...
2. `docs/研发计划_005_LLM_Player设计研究.md` — **当前阶段执行依据**:三大设计问题(Observation/Action/内部状态)的裁决与依据、后端接入与成本策略、WP13–WP20 路线。
...
5. `git log --oneline -20`;干活前跑 `dotnet test DramaBoard.slnx` 确认基线(206 绿;Local.slnx 220 绿,含 atelia 落盘集成)。
...
### Player.Llm(src/Player.Llm,WP13–WP19,零 NuGet,引用 Protocol+Host)
...
- **FirstBoard.Demo**:BCL-only console;Alice/Bob 可用 `--alice-backend/model`、`--bob-backend/model` 独立选择后端,相同 endpoint 仍共享实例;其余端点/env key/超时/turn 预算共用配置。每成功 turn 立即写 memory snapshot+turns.md,结束写中文 journal+内心轨迹。WP17–WP19 已跑真实混合场;输出在 git ignored `artifacts/wp15/`–`wp19/`。Windows 用户级 DeepSeek key/base URL 经 IDE 重启后已正确继承到终端,无需额外注入或增加 key。

## 4. 当前状态与后续计划(交接核心)

- **已完成**:WP0–WP19 全部;A1–A10 + S1–S14 两轮评审修复全清。关键 commit:δ=cd75fcc、α=992c5a2、β=a3f78a2、γ=392a63d、WP12=b16ca21、WP13=c5307a2、WP14=f3d0721、WP15=930ba78、WP16=d8d06ae、WP17=e5fd4ec、WP18=a082a50;WP19 commit 见当前 HEAD。
...
### 下一步:WP20 不可变 Scene Briefing

WP19 实现与真场资料:
- 用户确认方案 B 并要求先收窄:带 `TargetObjectId` 的 observe 只检查本人持有物;无目标仍观察现场。公共无主物/他人物品不在候选,越界 action.rejected。复用 ActorObservedEvent(新增可选目标),密信给 `object.inspected`+真伪+内容三条私有事实;没有新增动作/事件种类。
- 脚本场锁定候选、拒绝、事实私有性与所有权不变。首个混合场 100 events/24 turns/约 427s:Luna Bob 在酒馆主动检查密信并吸收内容,双方错身未交易。第二场把相遇约束移到固定角色 Goal 后 86 events/24 turns/约 442s,10min 会面;Alice 两币换到钥匙后主动检查钥匙,Bob 已先开箱且保留密信。两后端/两对象均自然使用目标观察,机制成立;结局暴露的是场景前提稳定性而非检查权限。
- “古董放桌上慢慢看”未硬塞进检查规则。后续候选是持有者 `put/drop` 到本地公共无主态→他人可检查,同时自然开放 take/盗取风险;通用 attribute/permission 系统继续后置。

WP20 方向(未否决按倾向推进):
1. 可变记忆整体替换会摘要掉稳定时限/会面约束。A=继续混入 memory/CharacterCard Goal;B=新增独立不可变 Scene Briefing/Directives,每轮固定渲染;C=Director 干预世界。**倾向 B**。
2. Briefing 只陈述角色稳定已知前提,不替角色选行动、不进世界 authority;脚本测试锁渲染与记忆替换隔离,真场验证会面不靠增加 turn 预算。
3. `put/drop`+公共检查作为后续动作表达力候选;005 §6 既定倾向仍有效:独白向观众展示、prompt/记忆用中文。
